...
This is a dockerized, Julia-based logistic regression model that can be used for binary and multiclass classification tasks. The purpose of this project is to provide a functional model that can be used as a template for building your own classification model.

The implementation contains a simple preprocessing pipeline which handles missing data and categorical features. The main prediction model is logistic regression. You may replace the preprocessing logic, or the logistic regression model, with your own custom logic. The model is implemented in Julia scripts for training and batch prediction tasks.

This implementation can be used on any classification dataset that follows the Ready Tensor specifications. You can read about the specifications here: https://docs.readytensor.ai/category/model-requirements
